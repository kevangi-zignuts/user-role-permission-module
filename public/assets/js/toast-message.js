...
$(document).ready(function () {
  ...
  // Check if error parameter exists and is true
  var errorParam = getUrlParameter('error');
  if (errorParam == '1') {
    var messageParam = getUrlParameter('message');
    var toastAnimationExample = document.querySelector('.error-message');
    var selectedType = 'text-danger';
    var selectedAnimation = 'animate__tada';
    toastAnimationExample.classList.add(selectedAnimation);
    toastAnimationExample.querySelector('.ti').classList.add(selectedType);
    var Message = document.querySelector('.toast-body-error');
    Message.innerText = messageParam;
    toastAnimation = new bootstrap.Toast(toastAnimationExample);
    toastAnimation.show();
  }
