          ...
          <form action="{{ route('roles.update', ['id' => $role->id]) }}" method="post"  id="formAuthentication" class="mb-3">
              ...
              <div class="select2-primary">
                <select id="select2Primary" class="select2 form-select" name="permissions[]" multiple>
                  @foreach ($permissions as $permission)
                    <option value="{{ $permission->id }}" value="{{ $permission->id }}" {{ $role->permission->contains($permission->id) ? 'selected' : '' }}>{{ $permission->permission_name }}</option>
                  @endforeach
                </select>
              </div>
            </div>
            <div class="row">
              <div class="mt-3">
                ...
                <button type="button" class="btn btn-label-secondary"><a href="{{ route('roles.index') }}">Cancle</a></button>
              </div>
            </div>
          </form>
        </div>
